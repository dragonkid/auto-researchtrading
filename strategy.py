    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (_lr.slope > 0 or (abs(_lr.slope) < 0.0006 and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and (_lr.slope < 0 or (abs(_lr.slope) < 0.0006 and bear_votes > bull_votes)):
                    target = -size
