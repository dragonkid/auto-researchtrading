...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed _persistence_mult voter weighting.
            # The 8-bar magnitude-weighted per-voter persistence multiplier in [0.7, 1.3]
            # is noise-amplifying: under perturbation each voter's signal value shifts,
            # the |sum(signal)|/sum(|signal|) ratio shifts, multiplier shifts; this
            # cascades through _bull_strong/_bear_strong to admission, conviction-margin,
            # and exit voter_bias — multiplicative noise compounding via voter aggregation.
            # _base_weights alone (with _wt_shift trend redistribution already capturing
            # the trend-vs-mean-revert component) provides stable voter weighting without
            # extra cross-bar state dependency. Code-structure removal: 21 lines + per-
            # symbol per-bar voter-sign history tuple list.
