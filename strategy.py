...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # 6 voters as continuous confidence: each voter outputs tanh((value - threshold) / scale) in [-1, +1]
            # Sum yields confidence in [-6, +6]. Bull/bear "votes" derived as |conf| with sign for thresholding.
            # Architectural: replaces hard binary thresholds with smooth confidence — eliminates per-voter noise flips at boundaries.
            ...
            _v1 = np.tanh((ret_short - 0.0) / dyn_threshold)
            _v2 = np.tanh((_ef - _es) / (_es * 0.005))
            _v3 = np.tanh((rsi - _rsi_thr) / 8.0)
            _v4 = np.tanh(_macd_h / 0.0006)
            _v5 = np.tanh(_lr.slope / 0.0003)
            _v6 = np.tanh(_ema_slope / 0.001)
            confidence = _v1 + _v2 + _v3 + _v4 + _v5 + _v6
            bull_votes = max(0.0, confidence)
            bear_votes = max(0.0, -confidence)
