    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural simplification: removed _ve_pressure (vol-of-vol
                # expansion exit pressure). Vol expansion correlates highly with
                # adverse price moves, which are already captured by
                # _sl_slope_pressure (slope-against) and _sl_pressure (ATR-based
                # stop). The "regime-shift detector" framing was ad-hoc; in
                # practice _vol_6/_vol_18 fires whenever recent 6-bar volatility
                # expands, which usually coincides with directional moves that
                # the existing stack already handles. Removing eliminates a
                # correlated exit-pressure term and reduces LOC.
