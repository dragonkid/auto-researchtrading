            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _sl_pressure >= 0.95 and _exit_pressure >= 1.0 and target != 0:
                    ...
                    if _exit_pressure >= _de_floor * _exit_thresh:
                        ...
                        # branch step4: LOW-VOL-GRIND amplification. The 0.50 concavity (step1)
                        # engaged RALLY (+0.002024, the low-vol grind where pullback losers are
                        # giveback-prone -> cutting faster helps) but SATURATED there (step2/3
                        # deeper magnitude/floor over-cut MIXED, the moderate-vol trending regime
                        # whose losers recover). The validated rally/mixed separator is VOL REGIME
                        # (rally low-vol grind, mixed moderate vol). Amplify the concavity for LOW
                        # vol_ratio (rally grind) by up to +0.30 (total 0.80) so rally losers get
                        # a deeper concave cut while mixed (moderate vol) keeps the 0.50 baseline.
                        # Continuous tanh on (0.9 - vol_ratio)/0.3 (full amp at vol_ratio<=0.6,
                        # fading to 0 by vol_ratio>=0.9); byte-identical for moderate-vol mixed.
                        _dr_low_vol_amp = max(0.0, min(1.0, np.tanh((0.9 - vol_ratio) / 0.3)))
                        _dr_loss_concave = _dr_loss_concave * (1.0 + 0.30 * _dr_low_vol_amp)
