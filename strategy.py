...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # EMA slope: median of 3 consecutive slope measurements (noise filter)
            # Each slope is (_ea[t] - _ea[t-LOOKBACK]) / _ea[t-LOOKBACK]
            # Median of 3 preserves responsiveness but filters single-bar noise spikes
            _ema_slopes = [(_ea[-1-i] - _ea[-EMA_SLOPE_LOOKBACK-i]) / _ea[-EMA_SLOPE_LOOKBACK-i] for i in range(3)]
            _ema_slope_med = sorted(_ema_slopes)[1]  # median of 3 values

            # 6 voters (ret_vshort removed: redundant with ret_short, adds noise channel without info)
            bull_votes = sum([ret_short > dyn_threshold, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, _ema_slope_med > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, _ema_slope_med < -0.0005])
