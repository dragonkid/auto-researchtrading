...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # RSI + EMA cross normalization also vol-adaptive: wider in calm for less boundary sensitivity
            _rsi_norm = (3.0 + 0.5 * max(0.0, min(1.0, (1.3 - vol_ratio) / 0.8))) * VOTE_SIGMOID_SCALE
            _ema_cross_norm = abs(_es) * 0.001 * VOTE_SIGMOID_SCALE * _vol_widen
            _voter_deltas_bull = [
                (ret_short - dyn_threshold) / max(dyn_threshold * VOTE_SIGMOID_SCALE, 1e-10),
                (_ef - _es) / max(_ema_cross_norm, 1e-10),
