...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Geometric-mean strong-sum aggregation: replaces quintic-weighted sum with
            # geometric mean of voter confidences. Topology: any single low-confidence voter
            # collapses the strong-sum (multiplicative penalty). Sum-aggregation lets one
            # confident voter compensate for two weak ones; geom-mean requires broad agreement.
            # Different decision architecture than the prior linear-additive sum.
            _bull_arr = np.array(_bull_confs)
            _bear_arr = np.array(_bear_confs)
            _bull_geom = np.exp(np.mean(np.log(_bull_arr)))
            _bear_geom = np.exp(np.mean(np.log(_bear_arr)))
            _bull_strong = max(0.0, _bull_geom - 0.5) * 15.0
            _bear_strong = max(0.0, _bear_geom - 0.5) * 15.0
