...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed _persistence_mult per-voter
            # magnitude-weighted persistence weighting. Maintained 8-bar rolling
            # history of raw voter signals per symbol, computed |sum|/sum(|.|)
            # persistence, weighted each voter by 0.7-1.3x. Redundant with
            # aggregate-level _recent_strongs 3-bar persistence (_min_bull_2,
            # _min_bear_2 entry gate) which catches noisy single-bar entries
            # at the strong-sum level. Per-voter granularity adds 18 LOC + new
            # per-symbol per-voter state dependency for noise-filtering that the
            # 3-bar strong-sum minimum already enforces structurally. Removing
            # eliminates the per-voter state and reverts to base voter weights.
