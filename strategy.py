...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Fixed RSI period=8 (architectural: eliminate dynamic-period noise channel)
            # Previously: period = round(6 + 2*rsi_trend_str) meant ±5bps noise on close
            # changes ret_long_lagged -> changes rsi_trend_str -> changes period -> changes all RSI diffs
            # Fixed period removes this cascading noise amplifier
            _rd = np.diff(closes[-9:])
