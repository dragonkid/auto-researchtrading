    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # (peak >= 1.6*_pp_min triggered 30% scale-down). _pp_pressure already
                # locks profits via giveback trailing when peak reaches _pp_min. The
                # take-profit harvest scaled down position BEFORE giveback occurred,
                # which prevents the position from capturing further upside in
                # extended trending regimes (rally/crash). _pp_pressure trailing is
                # responsive to actual price action (giveback), making preemptive
                # harvest a redundant size cut that competes with giveback locking.
                # Removed third exit-decision path; restored unified giveback-driven
                # exit semantics. Code-structure removal: 11 lines + new control flow.
