    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural simplification: removed conviction-margin SIZE modulation
                # on cold-entry path (was 0.06 * tanh(max(0,margin)/0.30), max +6% on
                # high-conviction entries). The 6% size delta is small relative to other
                # sizing multipliers (calm_boost up to 1.8x, sideways_boost up to 1.5x,
                # combined_mult cap up to 6.5x), and noise saturation in margin
                # computation likely makes the signal negligible. Simpler entry sizing.
