    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: price-confirmation gate on entry size.
                # Current bar's return (close vs prev close) provides the most
                # immediate information about market direction. When voters signal
                # one direction but the current bar moves the other way, there is
                # a short-term contradiction — the entry is "catching a falling
                # knife" or "shorting into strength." Attenuate first-bar size
                # smoothly via tanh on bar return * entry direction. Max attenuation
                # 0.15x. New cross-component data dependency: entry size depends on
                # current-bar return direction, orthogonal to all window-based voters.
                _bar_return = (mid - closes[-2]) / closes[-2]
                _price_confirm = 1.0 - 0.15 * max(0.0, np.tanh(-_bar_return / 0.003))
                _price_confirm_bear = 1.0 - 0.15 * max(0.0, np.tanh(_bar_return / 0.003))
                if _bull_strong >= _bull_strong_min and _bull_admit and _bull_persist_ok:
                    target = size * min(0.55, _entry_frac_dyn + _range_bull_adj) * _cooldown_factor * _bull_ct_atten * _bull_consensus_atten * _bull_quality_atten * _vol_entry_atten * _outcome_size_mult * _port_dd_atten * _tod_atten * _price_confirm
                elif _bear_strong >= _bear_strong_min and _bear_admit and _bear_persist_ok:
                    target = -size * min(0.55, _entry_frac_dyn + _range_bear_adj) * _cooldown_factor * _bear_ct_atten * _bear_consensus_atten * _bear_quality_atten * _vol_entry_atten * _outcome_size_mult * _port_dd_atten * _tod_atten * _price_confirm_bear
            elif current_pos != 0:
