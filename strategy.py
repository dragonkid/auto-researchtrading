        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if current_pos > 0 and rsi_exit > effective_ob:
                    ...
                    target = 0.0
                if target != 0 and ((abs(ret_long) < 0.025 and bars_held >= 8 and ((current_pos > 0 and _lr.slope < -0.0002) or (current_pos < 0 and _lr.slope > 0.0002))) or (current_pos > 0 and ret_long > 0.05 and _lr.slope < -0.0003)):
                    target = 0.0
