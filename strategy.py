...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # R2-based strength scale: noise-immune (uses HL2 linreg fit quality)
            # High R2 (strong linear trend) → larger size; low R2 (choppy) → moderate floor
            # Replaces noise-sensitive abs(ret_short)/threshold with noise-immune R2
            strength_scale = max(0.8, min(2.0, 0.8 + 1.2 * _r2))
