    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Regime-adaptive flip: harder to flip in strong trends (noise protection), easy in sideways (essential for profit)
                adaptive_flip_votes = FLIP_MIN_VOTES + int(cooldown_trend_strength > 0.8)
                if not in_cooldown and ((current_pos > 0 and bear_votes >= adaptive_flip_votes and trend_avg < 0) or (current_pos < 0 and bull_votes >= adaptive_flip_votes and trend_avg > 0)):
