...
# Multi-timeframe agreement sizing
MTF_AGREE_BOOST = 0.20          # boost when all 3 TFs agree
MTF_DISAGREE_DAMPEN = 0.15      # dampen when short vs long disagree
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Multi-timeframe agreement: short, medium, long momentum direction
            short_dir = 1 if ret_vshort > 0 else (-1 if ret_vshort < 0 else 0)
            med_dir = 1 if ret_med > 0 else (-1 if ret_med < 0 else 0)
            long_dir = 1 if ret_long > 0 else (-1 if ret_long < 0 else 0)
            mtf_mult = 1.0
            if short_dir == med_dir == long_dir and short_dir != 0:
                mtf_mult = 1.0 + MTF_AGREE_BOOST
            elif short_dir != 0 and long_dir != 0 and short_dir != long_dir:
                mtf_mult = 1.0 - MTF_DISAGREE_DAMPEN
            ...
            combined_mult = vol_scale * strength_scale * calm_boost * sideways_boost * dampened_cross_agree * vote_boost * vol_confirm_mult * mtf_mult
