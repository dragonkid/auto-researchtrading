        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if bars_held <= _entry_full_bars_dyn:
                    ...
                    # Branch step 6: dual-confirmation pace boost (slope_align AND pos_pnl>0).
                    # Both confirmations required to accelerate. Slope alignment alone
                    # (step 5) hurt bull where bull-position slopes are usually aligned
                    # but momentum already captured via standard 3-bar pace. Pos_pnl
                    # alone (steps 1,3,4) hurt rally where transient dips zero the boost.
                    # Require BOTH (slope-aligned trajectory AND realized winning pnl)
                    # for pace boost. Multiplicative AND-gate via product of two
                    # tanh-clamped factors. Reduces over-firing on either signal alone.
                    ...
                    _slope_align_si = max(0.0, np.tanh(_slope_si * _pos_dir_p / 0.0008))
                    _pnl_align_si = max(0.0, np.tanh(pos_pnl / abs(STOP_LOSS_PCT)))
                    _dual_conf = _slope_align_si * _pnl_align_si  # AND-gate
                    _pnl_pace_mult = 1.0 + 0.4 * _dual_conf
