...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _ret_short_lagged = (closes[-2] - closes[-adaptive_med - 1]) / closes[-adaptive_med - 1]
            strength_scale = max(0.6 + (STRENGTH_FLOOR_SIDEWAYS - 0.6) * (1.0 - min(abs(_ret_long_lagged) / STRENGTH_FLOOR_DECAY, 1.0)), min(2.0, (abs(_ret_short_lagged) / dyn_threshold) ** 0.85))
