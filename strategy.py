        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if target != 0 and self.peak_pnl[symbol] > 1.6 * _pp_min and _sl_pressure < 0.5:
                    ...
                    _deep_peak_tp = max(0.0, min(1.0, np.tanh((_tp_ratio - 3.2) / 0.5)))
                    _tp_scale = 0.30 * max(0.0, min(1.0, np.tanh((_tp_ratio - 1.6) / 0.6))) * _tp_trend_gate * (1.0 - 0.4 * _mae_clean * _trend_align_tp * _deep_peak_tp)
