...
MIN_VOTES = 4
FLIP_MIN_VOTES = 4
MIN_VOTE_MARGIN = 3   # net directional agreement required (bull-bear or bear-bull)
COOLDOWN_BARS = 1
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                _net_margin_bull = bull_votes - bear_votes
                _net_margin_bear = bear_votes - bull_votes
                if bull_votes >= MIN_VOTES and _net_margin_bull >= MIN_VOTE_MARGIN and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size * ENTRY_INITIAL_FRAC
                elif bear_votes >= MIN_VOTES and _net_margin_bear >= MIN_VOTE_MARGIN and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size * ENTRY_INITIAL_FRAC
                ...
                # Flip mechanism (4 votes + margin + trend_avg sign, vol-scaled accumulation)
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and (bear_votes - bull_votes) >= MIN_VOTE_MARGIN and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and (bull_votes - bear_votes) >= MIN_VOTE_MARGIN and trend_avg > 0)):
