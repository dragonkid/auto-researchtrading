...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural: loss-magnitude-scaled recovery window for _outcome_size_mult.
            # Original used flat 8-bar recovery regardless of prior loss magnitude — a
            # marginal -0.005 loss got the same 8-bar cooldown as a full -0.024 stop.
            # New: window scales 8..14 bars proportional to loss magnitude. Larger
            # losses get longer entry-size attenuation; small losses recover faster.
            # New cross-bar data dep: recovery window depends on prior loss magnitude
            # (was flat). _loss_only already encodes magnitude via tanh; reusing it
            # here unifies magnitude-conditioning at both attenuation depth (0.45 cap)
            # and time horizon (window scaling).
            _recovery_window = 8.0 + 6.0 * _loss_only
            _outcome_size_mult = 1.0 - 0.45 * max(0.0, 1.0 - _bars_since_exit / _recovery_window) * _loss_only
            in_cooldown = False
