...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _voter_signals_bull = [
                (ret_short - dyn_threshold) / max(dyn_threshold * 0.25, 1e-6),
                (_ef - _es) / (mid * 0.0010),
                (rsi - _rsi_thresh) / 5.0,
                (_macd_diff - 0.0003) / 0.00015,
                (_lr.slope - 0.00015) / 0.000125,
                (_ea_slope - 0.0005) / 0.00031,
            ]
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and _bull_strong >= STRONG_WEIGHT_MIN and _bull_certainty >= CERTAINTY_MIN and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
