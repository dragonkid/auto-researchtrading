...
# Linreg underwater flip parameters
LINREG_FLIP_SLOPE = 0.0004
LINREG_FLIP_LOSS_THRESHOLD = -0.003
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Linreg underwater flip: deterministic, targets losing positions
                if pos_pnl < LINREG_FLIP_LOSS_THRESHOLD and not in_cooldown:
                    if (current_pos > 0 and _lr.slope < -LINREG_FLIP_SLOPE and trend_avg < 0) or \
                       (current_pos < 0 and _lr.slope > LINREG_FLIP_SLOPE and trend_avg > 0):
                        target = -size if current_pos > 0 else size
                # Vote-based flip: strong signals only (5+ votes)
                if target != 0 and not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and trend_avg > 0)):
                    target = -size if current_pos > 0 else size
