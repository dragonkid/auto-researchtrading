    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: position-maturity flip gate. Flipping out of a young
                # position (just entered) is more likely a noise-driven reversal than
                # a real regime change. Smooth decay: full +25% strong-min tightening
                # at bars_held=0, zero at bars_held>=5. New data dependency: flip
                # strong-min scales with current-position age. Orthogonal to recency
                # gate (which measures bars-since-last-flip, this measures bars-in-current-pos).
                _maturity_factor = max(0.0, 1.0 - bars_held / 5.0)  # 1.0 fresh, 0.0 after 5 bars
                _bull_flip_min = _bull_strong_min * (1.0 + 0.20 * _flip_recency_factor + 0.25 * _maturity_factor)
                _bear_flip_min = _bear_strong_min * (1.0 + 0.20 * _flip_recency_factor + 0.25 * _maturity_factor)
