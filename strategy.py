...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Fused slope voter: MACD histogram AND linreg must agree (reduces noise-sensitive boundaries)
            _macd_bull = (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0002
            _macd_bear = (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0002
            _slope_bull = _macd_bull and _lr.slope > 0.00010
            _slope_bear = _macd_bear and _lr.slope < -0.00010

            bull_votes = sum([ret_short > dyn_threshold, ret_vshort > dyn_threshold * 0.70, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), _slope_bull, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, ret_vshort < -dyn_threshold * 0.70, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), _slope_bear, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
