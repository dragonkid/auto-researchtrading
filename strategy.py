...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            open_trend_avg = (TREND_GATE_MED_WEIGHT_SIDEWAYS - (TREND_GATE_MED_WEIGHT_SIDEWAYS - TREND_GATE_MED_WEIGHT_BASE) * cooldown_trend_strength) * ((opens[-1] - opens[-MED2_WINDOW]) / opens[-MED2_WINDOW]) + ((1.0 - TREND_GATE_MED_WEIGHT_SIDEWAYS) + (TREND_GATE_MED_WEIGHT_SIDEWAYS - TREND_GATE_MED_WEIGHT_BASE) * cooldown_trend_strength) * ((opens[-1] - opens[-LONG_WINDOW]) / opens[-LONG_WINDOW])

            bullish = bull_votes >= MIN_VOTES and (trend_bull or (abs(open_trend_avg) < TREND_GATE_DEADZONE and bull_votes > bear_votes))
            bearish = bear_votes >= MIN_VOTES and (trend_bear or (abs(open_trend_avg) < TREND_GATE_DEADZONE and bear_votes > bull_votes))
