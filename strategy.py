...
class Strategy:
    def __init__(self):
        ...
        self.bar_count = 0
        self.smoothed_trend = {}
        self.prev_bull_votes, self.prev_bear_votes = {}, {}
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            self.prev_bull_votes[symbol], self.prev_bear_votes[symbol] = bull_votes, bear_votes

            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and _prev_bull >= MIN_VOTES - 1 and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and _prev_bear >= MIN_VOTES - 1 and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
