...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _rd = np.diff((bd.history["high"].values[-(int(round(6 + 2 * rsi_trend_str)) + 1):] + bd.history["low"].values[-(int(round(6 + 2 * rsi_trend_str)) + 1):]) / 2.0)
            rsi = 100 - 100 / (1 + np.mean(np.maximum(_rd, 0)) / max(np.mean(np.maximum(-_rd, 0)), 1e-10))
            _rd_exit = np.diff((bd.history["high"].values[-8:] + bd.history["low"].values[-8:]) / 2.0)
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if target != 0:
                    _spp = 0.7 * pos_pnl + 0.3 * self.peak_pnl.get(symbol, pos_pnl)
                ...
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and _lr.slope < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and _lr.slope > 0)):
