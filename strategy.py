    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                _in_trend_regime = cooldown_trend_strength > 0.5
                _flip_min = 5 if _in_trend_regime else FLIP_MIN_VOTES
                _flip_dir = self.smoothed_trend[symbol] if _in_trend_regime else trend_avg
                if not in_cooldown and ((current_pos > 0 and bear_votes >= _flip_min and _flip_dir < 0) or (current_pos < 0 and bull_votes >= _flip_min and _flip_dir > 0)):
