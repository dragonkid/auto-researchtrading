...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            self.slow_trend[symbol] = 0.60 * trend_avg + 0.40 * self.slow_trend.get(symbol, trend_avg)
            entry_trend = 0.5 * (self.smoothed_trend[symbol] + self.slow_trend[symbol])
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (entry_trend > 0 or (abs(entry_trend) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and (entry_trend < 0 or (abs(entry_trend) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
