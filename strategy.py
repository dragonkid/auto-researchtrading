...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _confluence_adj = 0.0
            # Architectural simplification: removed Kaufman efficiency ratio (_er_adj) entry adj.
            # The -0.025 max attenuation in deep chop (ER < 0.15) duplicates the deep-chop
            # suppression already provided by sideways_boost (max +50% in chop scales positively
            # but is offset by vol_ratio cap) and the noise-filter pipeline (_bull_quality_atten,
            # _bull_consensus_atten, persistence gates). The narrow ER<0.15 activation band
            # rarely fires meaningfully outside genuine deep chop; in those bars, the existing
            # attenuator stack already cuts size. Removing eliminates redundant deep-chop
            # multiplier and saves 9 lines (path/net/ratio/adj computation).
            _er_adj = 0.0
