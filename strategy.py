...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            recent_entries = sum(1 for b in self.entry_history.get(symbol, []) if self.bar_count - b <= 8)
            freq_dampener = 1.0 - 0.4 * min(recent_entries / 4.0, 1.0)
            size = equity * BASE_POSITION_SIZE * combined_mult * freq_dampener
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    ...
                    self.entry_history[symbol] = [b for b in self.entry_history.get(symbol, []) if self.bar_count - b <= 16] + [self.bar_count]
