...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Smoothed strength: 3-bar average of absolute momentum for noise-immune sizing
            _str_vals = [abs((smoothed_closes[-1-_k] - _med_ref_med) / _med_ref_med) for _k in range(3)]
            _smooth_str = np.mean(_str_vals)
            strength_scale = max(0.6 + (STRENGTH_FLOOR_SIDEWAYS - 0.6) * (1.0 - min(abs(ret_long) / STRENGTH_FLOOR_DECAY, 1.0)), min(2.0, (_smooth_str / dyn_threshold) ** 0.85))
