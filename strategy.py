...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Vol-adaptive voter thresholds: scale slope/MACD thresholds by sqrt(vol_ratio)
            # Mechanism: in high vol, noise magnitude is larger so thresholds must widen
            # to avoid noise-triggered flips. In calm, values are smaller but also cleaner.
            _vol_thresh_scale = max(0.7, min(1.6, vol_ratio ** 0.5))
            _linreg_thresh = 0.00015 * _vol_thresh_scale
            _ema_slope_thresh = 0.0005 * _vol_thresh_scale
            _macd_thresh = 0.0003 * _vol_thresh_scale

            # 6 voters (ret_vshort removed: redundant with ret_short, adds noise channel without info)
            bull_votes = sum([ret_short > dyn_threshold, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > _macd_thresh, _lr.slope > _linreg_thresh, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > _ema_slope_thresh])
            bear_votes = sum([ret_short < -dyn_threshold, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -_macd_thresh, _lr.slope < -_linreg_thresh, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -_ema_slope_thresh])
