...
# Voter deadzone: ret_short/ret_vshort abstain when within this fraction of threshold
# Reduces noise sensitivity at boundary without changing threshold level
VOTER_DEADZONE_FRAC = 0.15
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Deadzone: ret_short/ret_vshort only count as votes when clearly beyond threshold
            _dz_short = dyn_threshold * (1.0 + VOTER_DEADZONE_FRAC)
            _dz_vshort = dyn_threshold * 0.75 * (1.0 + VOTER_DEADZONE_FRAC)
            bull_votes = sum([ret_short > _dz_short, ret_vshort > _dz_vshort, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005])
            bear_votes = sum([ret_short < -_dz_short, ret_vshort < -_dz_vshort, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
