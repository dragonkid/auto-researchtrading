...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Vol-adaptive smoothing: more in calm (span~3.5), less in choppy (span~2)
            # vol_ratio < 0.7 (calm): alpha=0.44 (span~3.5); vol_ratio > 1.2 (choppy): alpha=0.63 (span~2.3)
            _smooth_alpha = 0.44 + 0.19 * max(0.0, min(1.0, (vol_ratio - 0.7) / 0.5))
