...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Vol-adaptive cooldown floor: low vol -> longer cooldown (prevents noise re-entry).
            # In rally (low vol, low ret_long), cooldown was 0 bars -> immediate re-entry adds noise paths.
            _cooldown_floor = 1.0 + 1.0 * max(0.0, min(1.0, (0.7 - vol_ratio) / 0.4))
            in_cooldown = (self.bar_count - self.exit_bar.get(symbol, -999)) < max(_cooldown_floor, COOLDOWN_BARS * cooldown_trend_strength)
