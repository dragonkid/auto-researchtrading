...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Smooth cooldown_factor (tanh decay over trend-scaled window) with
            # loss-only outcome-conditioned _cd_window stretch (sole loss attenuator).
            # Architectural simplification: removed _outcome_size_mult (separate 8-bar
            # loss-only multiplicative attenuator). Redundant with _cooldown_factor's
            # _cd_window stretch (already extends ramp by 60% on loss). Two multiplicative
            # loss attenuators on the same _loss_only signal compose: first-bar size after
            # a loss got attenuated by both ~tanh(1/2.4)≈0.41 (cooldown) AND 0.55 (outcome) =
            # ~0.23x raw. The cooldown stretch alone provides correct attenuation gradient;
            # the outcome multiplier was double-counting the same loss-recovery window on
            # a different timescale (8 bars vs ~2-4 bar cooldown). Code-structure removal.
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_strong >= _bull_strong_min and _bull_admit and _bull_persist_ok:
                    target = size * min(0.55, _entry_frac_dyn + _range_bull_adj) * _cooldown_factor * _bull_ct_atten * _bull_consensus_atten * _bull_quality_atten * _vol_entry_atten * _port_dd_atten * _tod_atten
                elif _bear_strong >= _bear_strong_min and _bear_admit and _bear_persist_ok:
                    target = -size * min(0.55, _entry_frac_dyn + _range_bear_adj) * _cooldown_factor * _bear_ct_atten * _bear_consensus_atten * _bear_quality_atten * _vol_entry_atten * _port_dd_atten * _tod_atten
            elif current_pos != 0:
