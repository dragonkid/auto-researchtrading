    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural primitive change: replace piecewise-linear pp_activation
                # ramp (two hard breakpoints at 0.95 and 1.04) with smooth tanh sigmoid
                # centered at 0.995 with matched slope (~11.1 per unit at center → tanh
                # scale 5.55). Eliminates two boundary discontinuities that perturbed price
                # could cross repeatedly. LOC-reducing change.
                _pp_ratio = self.peak_pnl[symbol] / max(_pp_min, 1e-6)
                _pp_activation = 0.5 * (1.0 + np.tanh((_pp_ratio - 0.995) * 5.55))
