...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed vol-adaptive close smoothing.
            # Same rationale as smoothed_trend (line 202-203): EMA state propagation
            # amplifies noise. Use raw closes; voters that need smoothing have it built-in
            # (EMA voter, MACD, slope_16) — extra closes-layer was redundant.
            ...
            _med_ref_short = np.median(closes[-SHORT_WINDOW - 2: -SHORT_WINDOW + 3])
            _med_ref_med = np.median(closes[-adaptive_med - 2: -adaptive_med + 3])
            ret_vshort = (closes[-1] - _med_ref_short) / _med_ref_short
