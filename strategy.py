...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # loss-only outcome-conditioned stretch (cd_window stretches with loss).
            # Architectural simplification: removed _outcome_size_mult (additional
            # 0.45x first-bar size attenuator gated on _loss_only × time-since-exit).
            # Redundant with _cooldown_factor's own loss-stretch (cd_window grows by
            # up to 60% with loss-magnitude, lengthening the smooth cooldown ramp).
            # Both attenuate fresh re-entries after loss-exits; _outcome_size_mult was
            # the second post-loss attenuator on top of the time-decay cooldown.
            # Removing eliminates the multiplicative double-counting on the ~8-bar
            # post-loss window. Code-structure removal: 1 line + multiplier removed
            # from both entry paths.
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_strong >= _bull_strong_min and _bull_admit and _bull_persist_ok:
                    target = size * min(0.55, _entry_frac_dyn + _range_bull_adj) * _cooldown_factor * _bull_ct_atten * _bull_consensus_atten * _bull_quality_atten * _vol_entry_atten * _port_dd_atten
                elif _bear_strong >= _bear_strong_min and _bear_admit and _bear_persist_ok:
                    target = -size * min(0.55, _entry_frac_dyn + _range_bear_adj) * _cooldown_factor * _bear_ct_atten * _bear_consensus_atten * _bear_quality_atten * _vol_entry_atten * _port_dd_atten
            elif current_pos != 0:
