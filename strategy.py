...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        equity_for_sizing = min(equity, self.smoothed_equity)
        self.bar_count += 1
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _trend_str = min(abs(_ret_long_lagged) / STRENGTH_FLOOR_DECAY, 1.0)
            strength_scale = (STRENGTH_FLOOR_SIDEWAYS - (STRENGTH_FLOOR_SIDEWAYS - 0.6) * _trend_str) * (1.0 - _trend_str) + min(2.0, (abs(ret_short) / dyn_threshold) ** 0.85) * _trend_str
            ...
            size = equity_for_sizing * BASE_POSITION_SIZE * combined_mult
