...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            ret_long_lag = (closes[-2] - closes[-(LONG_WINDOW + 1)]) / closes[-(LONG_WINDOW + 1)]
            ...
            rsi_trend_str = min(abs(ret_long_lag) / RSI_TREND_BIAS_DECAY, 1.0)
            rsi = calc_rsi(closes, int(round(6 + 2 * rsi_trend_str)))
            rsi_thresh = 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long_lag > 0 else 1.0)
