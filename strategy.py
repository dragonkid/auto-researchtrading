    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _spp < 0 and rsi_trend_str > 0.5:
                    _uw = 1.5 * min(1.0, abs(pos_pnl) / 0.012)
                    effective_ob, effective_os = effective_ob + _uw, effective_os - _uw
                if vol_ratio < 0.55 and _spp > 0.01 and ((current_pos > 0 and _lr.slope > 0) or (current_pos < 0 and _lr.slope < 0)):
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    ...
                    self.smoothed_pos_pnl[symbol] = 0.0
