    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Lag-1 autocorrelation of recent log-returns: trending (positive) vs mean-rev (negative).
                # Side-aware: positive AR amplifies (momentum continuation more likely), negative AR attenuates (mean-rev).
                _ar_rets = np.diff(np.log(closes[-25:])); _ar1 = np.tanh((_ar_rets[1:] * _ar_rets[:-1]).mean() / max((_ar_rets[1:] ** 2).mean(), 1e-10) / 0.3)
                if _bull_strong >= _bull_strong_min and _bull_admit and _bull_persist_ok:
                    target = size * min(0.55, _entry_frac_dyn + _range_bull_adj) * _cooldown_factor * _bull_ct_atten * _bull_consensus_atten * _bull_quality_atten * _vol_entry_atten * _outcome_size_mult * _port_dd_atten * _tod_atten * (1.0 + 0.08 * _ar1)
                elif _bear_strong >= _bear_strong_min and _bear_admit and _bear_persist_ok:
                    target = -size * min(0.55, _entry_frac_dyn + _range_bear_adj) * _cooldown_factor * _bear_ct_atten * _bear_consensus_atten * _bear_quality_atten * _vol_entry_atten * _outcome_size_mult * _port_dd_atten * _tod_atten * (1.0 + 0.08 * _ar1)
            elif current_pos != 0:
