    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # amplifier. Last session's joint-removal experiment showed sideways +0.019
                # came from _chop_amp removal alone — the chop amplification of own-side
                # subtraction was a slight DRAG on sideways (over-amplified retain-pos
                # signal in chop produces sticky positions through reversal noise). The
                # opposite-side ADDITION via _opp_atten remains load-bearing for trend
                # regimes (crash/rally counter-trend handling). Code-structure removal:
                # 1 amplifier removed from bilateral voter_bias formula.
