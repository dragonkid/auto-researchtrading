...
class Strategy:
    def __init__(self):
        ...
        self.bar_count = 0
        self.smoothed_trend = {}
        self.prev_bull_voters, self.prev_bear_voters = {}, {}
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _bull_v = [ret_short > dyn_threshold, ret_vshort > dyn_threshold * 0.70, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, mid > np.max(closes[-(DONCHIAN_PERIOD+1):-1]) * 1.004, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005]
            _bear_v = [ret_short < -dyn_threshold, ret_vshort < -dyn_threshold * 0.70, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, mid < np.min(closes[-(DONCHIAN_PERIOD+1):-1]) * 0.9975, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005]
            _pb, _pe = self.prev_bull_voters.get(symbol, _bull_v), self.prev_bear_voters.get(symbol, _bear_v)
            bull_votes = sum(c and p for c, p in zip(_bull_v, _pb))
            bear_votes = sum(c and p for c, p in zip(_bear_v, _pe))
            self.prev_bull_voters[symbol], self.prev_bear_voters[symbol] = _bull_v, _bear_v
