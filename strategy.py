...
class Strategy:
    def __init__(self):
        ...
        # Exp4: sustain the Exp3 streak-ct shrink through scale-in (cached at entry,
        # deterministic). Keeps a streak-opened counter-trend entry smaller for the
        # whole hold instead of ramping back to un-shrunk `size` after bar 1.
        self._streak_ct_held = {}
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_ready and _bull_admit:
                    ...
                    self._streak_ct_held[symbol] = _streak_ct_shrink_bull  # Exp4: cache for scale-in sustain
                    ...
                    self._vol_shrink_held[symbol] = _vol_entry_spike  # Exp9: cache for scale-in sustain
                    self._streak_ct_held[symbol] = _streak_ct_shrink_bear  # Exp4: cache for scale-in sustain
            elif current_pos != 0:
                ...
                if bars_held <= _entry_full_bars_dyn:
                    ...
                    # Exp4: sustain the Exp3 streak-ct shrink through scale-in (cached at
                    # entry, deterministic). Keeps a streak-opened ct entry smaller for the
                    # whole hold instead of ramping back to un-shrunk `size` after bar 1.
                    _streak_ct_held = self._streak_ct_held.get(symbol, 1.0)
                    full_target = (size if current_pos > 0 else -size) * _conc_held * _vol_held * _streak_ct_held
                ...
                if target == 0:
                    if current_pos != 0:
                        ...
                        if _exit_pnl_signed < 0:
                            ...
                            self._loss_streak = 0
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self._smoothed_pnl, self._mae, self._exit_press_ema, self._voter_bias_ema, self._target_ema, self._conc_shrink_held, self._vol_shrink_held, self._streak_ct_held):
                        _d.pop(symbol, None)
