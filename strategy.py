...
STRONG_WEIGHT_MIN = 1.50  # 1.75 * 6/7 (scaled to 6 voters from 7)
MIN_VOTES = 2.50  # 2.92 * 6/7 (scaled to 6 voters)
FLIP_MIN_VOTES = 2.40  # 2.80 * 6/7 (scaled to 6 voters)
COOLDOWN_BARS = 1
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed 7th VWAP voter (-9 LOC, -1 voter).
            # VWAP voter used volume * price (only volume-dependent voter); rest are
            # price-derived. _vwap_wt was already scaled 0.55..1.05 (smaller than other
            # voters). Removing tests whether the volume channel adds load-bearing signal
            # or just adds correlated noise via shared HL2 inputs with EMA/MACD/slope voters.
            # 6 voters now; weights/normalization re-aligned. Code-structure removal.
            _voter_signals_bull = [
            ...
            # VWAP voter weight removed (voter removed above).
