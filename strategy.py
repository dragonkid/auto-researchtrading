...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _low_cap = MAX_COMBINED_MULT_LOW_VOL - 3.0 * max(0.0, min(1.0, (vol_ratio - MAX_COMBINED_VOL_LOW) / (MAX_COMBINED_VOL_HIGH - MAX_COMBINED_VOL_LOW)))
            _trans = max(0.0, min(1.0, (vol_ratio - (MAX_COMBINED_VOL_HIGH - 0.1)) / 0.2))
            combined_mult = min(combined_mult, (1.0 - _trans) * _low_cap + _trans * MAX_COMBINED_MULT_HIGH_VOL + MAX_COMBINED_TREND_BOOST * (1.0 - rsi_trend_str ** 0.85))
