...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # RSI bias direction: smooth tanh transition over ret_long (was hard sign flip — noise channel near ret_long~0)
            _rsi_bias_dir = -np.tanh(ret_long / 0.01)
            _rsi_thresh = 50 + RSI_TREND_BIAS * rsi_trend_str * _rsi_bias_dir
            bull_votes = sum([ret_short > dyn_threshold, _ef > _es, rsi > _rsi_thresh, (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, _ef < _es, rsi < _rsi_thresh, (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
