    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: volume-conviction exit-pressure scale.
                # When current bar's volume materially exceeds the 12-bar median
                # (high-conviction move), modestly amplify cumulative non-SL exit
                # pressure — institutional volume on adverse moves is a trustworthy
                # exit signal. When volume is weak (drift), modestly attenuate.
                # Continuous tanh on log-volume-ratio; bounded multiplier ~[0.92, 1.08].
                # Stop-loss component (_sl_pressure) is excluded from this scaling —
                # SL is entry-anchored protective and must not depend on volume.
                _vol_recent = bd.history["volume"].values[-13:-1]
                _vol_med = max(np.median(_vol_recent), 1e-6)
                _vol_log_ratio = np.log(max(bd.history["volume"].values[-1], 1e-6) / _vol_med)
                _vol_exit_scale = 1.0 + 0.08 * np.tanh(_vol_log_ratio / 0.5)
                _non_sl_pressure = _w_slope * _sl_slope_pressure + _w_pp * _pp_pressure + _w_time * _time_pressure
                _exit_pressure = _sl_pressure + _non_sl_pressure * _vol_exit_scale
