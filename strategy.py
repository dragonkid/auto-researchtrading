...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # EMA slope: average of last 3 slope readings (multi-point noise reduction)
            # Each reading is (ema[i] - ema[i-EMA_SLOPE_LOOKBACK]) / ema[i-EMA_SLOPE_LOOKBACK]
            # This preserves responsiveness (still uses recent data) while smoothing point noise
            _ema_slopes = np.array([
                (_ea[-1 - k] - _ea[-EMA_SLOPE_LOOKBACK - 1 - k]) / _ea[-EMA_SLOPE_LOOKBACK - 1 - k]
                for k in range(3)
            ])
            _avg_ema_slope = np.mean(_ema_slopes)

            # 6 voters (ret_vshort removed: redundant with ret_short, adds noise channel without info)
            bull_votes = sum([ret_short > dyn_threshold, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, _avg_ema_slope > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, _avg_ema_slope < -0.0005])
