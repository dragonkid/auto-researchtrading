...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural: voter-coherence size modulator (gentle coefficients).
            # Reduces position size continuously as voter dispersion grows. Mechanism validated
            # in prior experiment (bull stab +0.027, crash +0.017) but prior coefficients too
            # aggressive (1.10 - 0.875*disp). Gentler scale: 0.96 - 0.08*disp keeps returns intact.
            _voter_disp = float(np.std(_bull_confs))
            _coh_size_mult = 0.96 - 0.08 * min(_voter_disp / 0.30, 1.0)
            combined_mult *= _coh_size_mult
