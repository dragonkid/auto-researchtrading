...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _rd = np.diff(closes[-(_rsi_period + 1):])
            rsi = 100 - 100 / (1 + np.mean(np.maximum(_rd, 0)) / max(np.mean(np.maximum(-_rd, 0)), 1e-10))
            _rd_prior = np.diff(closes[-(_rsi_period + 2):-1])
            rsi_prior = 100 - 100 / (1 + np.mean(np.maximum(_rd_prior, 0)) / max(np.mean(np.maximum(-_rd_prior, 0)), 1e-10))
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if current_pos > 0 and max(rsi, rsi_prior) > effective_ob:
                    target = 0.0
                elif current_pos < 0 and min(rsi, rsi_prior) < effective_os:
                    target = 0.0
