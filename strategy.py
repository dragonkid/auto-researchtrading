    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Flip mechanism: require trend_avg magnitude > margin (not just sign)
                # Prevents noise-driven flips when trend is ambiguous (near zero)
                _flip_trend_margin = TREND_GATE_DEADZONE * 0.5  # half of entry deadzone
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and trend_avg < -_flip_trend_margin) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and trend_avg > _flip_trend_margin)):
