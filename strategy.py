    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # New control flow at SL decision boundary. Once peak_pnl has reached
                # meaningful territory (>= 0.4 * _pp_min ~ 1% profit), the effective
                # stop progressively tightens from full _stop_abs toward 0.5*_stop_abs.
                # Mechanism: positions that have shown decent profit have proven their
                # initial entry thesis — but if they're now reverting toward entry,
                # the reversal evidence is mounting. Tighten the stop to limit drawdown
                # on the now-reverting position. Smooth tanh ramp on peak_pnl/_pp_min;
                # no binary boundary. New cross-bar data dep: SL decision now depends
                # on peak_pnl history, not just current pos_pnl. Continuous.
                _stop_tighten_t = max(0.0, min(1.0, np.tanh((self.peak_pnl.get(symbol, 0.0) / max(_pp_min, 1e-6) - 0.4) / 0.6)))
                _stop_eff = _stop_abs * (1.0 - 0.5 * _stop_tighten_t)
                _loss = -pos_pnl
