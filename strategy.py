...
STRONG_WEIGHT_MIN = 1.25  # required sum (scaled with voter set: 5 voters, max strong-sum ~5.0)
MIN_VOTES = 2.1            # scaled from 2.5 with 5 voters (preserves admission ratio)
FLIP_MIN_VOTES = 2.0       # scaled from 2.4 with 5 voters (preserves flip admission ratio)
COOLDOWN_BARS = 1
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed 6th voter (_ea_slope, EMA-22 slope).
            # _ea_slope and _lr_slope (16-bar log-price slope) both measure trend slope
            # at adjacent windows; voter set is redundant. Removing the more lagged of
            # the two (EMA-smoothed slope adds extra lag to LR slope) reduces correlated
            # noise in strong-sum and reduces effective LOC. Voter weights re-normalized
            # to sum=5.0 (was 6.0).
            _voter_signals_bull = [
            ...
            _voter_weights = (0.80, 1.25, 1.10, 1.00, 0.85)
