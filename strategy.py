...
class Strategy:
    def __init__(self):
        ...
        # Exp1 (architectural, this session): sustain the multi-day COUNTER-TREND entry
        # shrink (_bull_ct_vlong/_bear_ct_vlong, line ~933) through scale-in. That shrink
        # cuts first-bar size up to 0.40x for an entry AGAINST the 96-bar trend (mixed's
        # wrong-side longs in a down year; rally pullback shorts; crash dead-cat longs),
        # but it is FIRST-BAR ONLY -- scale-in (line ~1694) ramps the position back to
        # full `size` over 3 bars, undoing the shrink so the held wrong-side position
        # reaches full net exposure by bar 3. Caching the entry-time ct_vlong shrink and
        # applying it to full_target (same pattern as _conc_shrink_held/_vol_shrink_held)
        # keeps the wrong-side position at ~0.60x for the WHOLE hold -> smaller unrealized
        # loss on the losing counter-trend book -> higher Sharpe/calmar in mixed (binding)
        # + rally (ct shorts). Trend-aligned entries (ret_vlong*pos_dir>0 -> gate 0 ->
        # shrink 1.0) and low-ret_vlong sideways cache 1.0 -> byte-identical. Deterministic
        # (set once at entry from the entry bar's ret_vlong). Reset on full exit; default 1.0.
        self._ct_vlong_shrink_held = {}
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_ready and _bull_admit:
                    ...
                    self._ct_vlong_shrink_held[symbol] = _bull_ct_vlong  # Exp1: sustain ct shrink through scale-in
                    ...
                    self._vol_shrink_held[symbol] = _vol_entry_spike  # Exp9: cache for scale-in sustain
                    self._ct_vlong_shrink_held[symbol] = _bear_ct_vlong  # Exp1: sustain ct shrink through scale-in
            elif current_pos != 0:
                ...
                if bars_held <= _entry_full_bars_dyn:
                    ...
                    # Exp1 (architectural): sustain the multi-day counter-trend entry
                    # shrink through scale-in (cached at entry, deterministic, same
                    # pattern as _conc_held/_vol_held). Keeps a wrong-side entry at its
                    # entry-time reduced commitment for the whole hold instead of ramping
                    # back to un-shrunk `size` over 3 bars. Default 1.0 (no effect) if
                    # trend-aligned/uncached.
                    _ct_vlong_held = self._ct_vlong_shrink_held.get(symbol, 1.0)
                    full_target = (size if current_pos > 0 else -size) * _conc_held * _vol_held * _ct_vlong_held
                ...
                if target == 0:
                    if current_pos != 0:
                        ...
                        if _exit_pnl_signed < 0:
                            ...
                            self._loss_streak = 0
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self._smoothed_pnl, self._mae, self._exit_press_ema, self._voter_bias_ema, self._target_ema, self._conc_shrink_held, self._vol_shrink_held, self._ct_vlong_shrink_held, self._pnl_path):
                        _d.pop(symbol, None)
