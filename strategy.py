...
# Confidence scoring parameters (replaces binary vote counting)
# Each voter contributes a continuous confidence [0, 1] based on how far past threshold
CONF_HALFWIDTH = 1.5  # signal at 1.5x threshold → confidence 0.75 (sigmoid midpoint at 1x)
CONF_ENTRY_THRESHOLD = 3.2  # sum of confidences needed for entry (replaces MIN_VOTES=4)
CONF_FLIP_THRESHOLD = 3.2  # sum of confidences needed for flip

# Vote / cooldown (retained for compatibility)
MIN_VOTES = 4
FLIP_MIN_VOTES = 4
COOLDOWN_BARS = 1
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Continuous confidence scoring: each voter contributes sigmoid(distance_past_threshold)
            # This reduces noise sensitivity at boundaries (near-threshold → ~0.5 instead of 0/1 flip)
            def _conf(value, threshold):
                """Sigmoid confidence: 0.5 at threshold, approaches 1.0 far past it."""
                x = (abs(value) / max(abs(threshold), 1e-10) - 1.0) / (CONF_HALFWIDTH - 1.0)
                return 1.0 / (1.0 + np.exp(-4.0 * x))  # steep sigmoid centered at threshold

            _rsi_threshold = 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0)
            _macd_hist = (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid
            _ema_slope_val = (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK]

            # Bull confidences (continuous [0, 1] per voter)
            bull_conf = (
                _conf(ret_short, dyn_threshold) * (1.0 if ret_short > 0 else 0.0) +
                _conf(ret_vshort, dyn_threshold * 0.75) * (1.0 if ret_vshort > 0 else 0.0) +
                _conf(_ef - _es, abs(_es) * 0.001) * (1.0 if _ef > _es else 0.0) +
                _conf(rsi - _rsi_threshold, 5.0) * (1.0 if rsi > _rsi_threshold else 0.0) +
                _conf(_macd_hist, 0.0003) * (1.0 if _macd_hist > 0 else 0.0) +
                _conf(_lr.slope, 0.00015) * (1.0 if _lr.slope > 0 else 0.0) +
                _conf(_ema_slope_val, 0.0005) * (1.0 if _ema_slope_val > 0 else 0.0)
            )
            # Bear confidences
            bear_conf = (
                _conf(ret_short, dyn_threshold) * (1.0 if ret_short < 0 else 0.0) +
                _conf(ret_vshort, dyn_threshold * 0.75) * (1.0 if ret_vshort < 0 else 0.0) +
                _conf(_ef - _es, abs(_es) * 0.001) * (1.0 if _ef < _es else 0.0) +
                _conf(_rsi_threshold - rsi, 5.0) * (1.0 if rsi < _rsi_threshold else 0.0) +
                _conf(_macd_hist, 0.0003) * (1.0 if _macd_hist < 0 else 0.0) +
                _conf(_lr.slope, 0.00015) * (1.0 if _lr.slope < 0 else 0.0) +
                _conf(_ema_slope_val, 0.0005) * (1.0 if _ema_slope_val < 0 else 0.0)
            )
            # Keep integer votes for compatibility with flip/trend logic
            bull_votes = sum([ret_short > dyn_threshold, ret_vshort > dyn_threshold * 0.75, _ef > _es, rsi > _rsi_threshold, _macd_hist > 0.0003, _lr.slope > 0.00015, _ema_slope_val > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, ret_vshort < -dyn_threshold * 0.75, _ef < _es, rsi < _rsi_threshold, _macd_hist < -0.0003, _lr.slope < -0.00015, _ema_slope_val < -0.0005])
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Flip mechanism (confidence threshold + trend_avg sign, vol-scaled accumulation)
                if not in_cooldown and ((current_pos > 0 and bear_conf >= CONF_FLIP_THRESHOLD and trend_avg < 0) or (current_pos < 0 and bull_conf >= CONF_FLIP_THRESHOLD and trend_avg > 0)):
