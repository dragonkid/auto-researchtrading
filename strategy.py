        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if current_pos > 0 and _ret_long_lagged > 0.02:
                    ...
                    effective_os -= _tw
                self.rsi_exit_ema[symbol] = 0.5 * rsi_exit + 0.5 * self.rsi_exit_ema.get(symbol, rsi_exit)
                if current_pos > 0 and self.rsi_exit_ema[symbol] > effective_ob:
                    target = 0.0
                elif current_pos < 0 and self.rsi_exit_ema[symbol] < effective_os:
                    target = 0.0
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self.rsi_exit_ema):
                        _d.pop(symbol, None)
                    self.exit_bar[symbol] = self.bar_count
                elif current_pos == 0 or (target > 0 and current_pos < 0) or (target < 0 and current_pos > 0):
                    self.entry_prices[symbol], self.peak_pnl[symbol], self.entry_bar[symbol] = mid, 0.0, self.bar_count
                    self.rsi_exit_ema.pop(symbol, None)
