...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _ni_bull = _lr.slope > 0.00015 or mid > np.max(closes[-(DONCHIAN_PERIOD+1):-1]) * 1.004
            _ni_bear = _lr.slope < -0.00015 or mid < np.min(closes[-(DONCHIAN_PERIOD+1):-1]) * 0.9975
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and _ni_bull and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and _ni_bear and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
