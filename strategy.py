        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_strong >= _bull_strong_min and _bull_admit and _bull_persist_ok:
                    # Architectural: conviction-margin entry skip. Marginal entries
                    # (strong-sum just barely above admission threshold) whipsaw
                    # frequently. Skip entries with margin < 0.04, admit fully above
                    # margin=0.10. Smooth band [0.04, 0.10]. New gate at entry boundary
                    # using existing bull_margin. Architectural because it adds a
                    # new control-flow branch at the entry decision point.
                    _skip_factor = max(0.0, min(1.0, (0.10 - _bull_margin) / 0.06))
                    if _skip_factor > 0.99:
                        target = 0.0
                    else:
                    ...
                    _skip_factor = max(0.0, min(1.0, (0.10 - _bear_margin) / 0.06))
                    if _skip_factor > 0.99:
                        target = 0.0
                    else:
