...
# Directional vote-margin gate: bull_votes - bear_votes >= MIN_VOTE_MARGIN
# Calibrated to match approx old MIN_VOTES=2.5 semantics on average:
# old admitted bull_votes-bear_votes >= -1.0; new requires actual directional consensus.
MIN_VOTE_MARGIN = 0.6
FLIP_VOTE_MARGIN = 0.4  # slightly looser for protective flips
COOLDOWN_BARS = 1
COOLDOWN_BARS = 1
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: directional vote MARGIN gate replaces asymmetric count gate.
                # Old: bull_votes >= 2.5 admits even slight bear leans (since bull+bear=6 always).
                # New: bull_votes - bear_votes >= MIN_VOTE_MARGIN requires actual directional consensus.
                # The margin is symmetric: same threshold for both sides, removes count-asymmetry.
                _vote_margin = bull_votes - bear_votes
                if _vote_margin >= MIN_VOTE_MARGIN and _bull_strong >= _strong_min and (_trend_biased > 0 or (abs(_trend_biased) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size * ENTRY_INITIAL_FRAC
                elif -_vote_margin >= MIN_VOTE_MARGIN and _bear_strong >= _strong_min and (_trend_biased < 0 or (abs(_trend_biased) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size * ENTRY_INITIAL_FRAC
                ...
                # Flip mechanism (vote margin + trend_avg sign, vol-scaled)
                if not in_cooldown and ((current_pos > 0 and (bear_votes - bull_votes) >= FLIP_VOTE_MARGIN and _bear_strong >= _strong_min and trend_avg < 0) or (current_pos < 0 and (bull_votes - bear_votes) >= FLIP_VOTE_MARGIN and _bull_strong >= _strong_min and trend_avg > 0)):
