    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if current_pos < 0:
                    pos_pnl = -pos_pnl
                tighten_pnl = (closes[-2] - self.entry_prices[symbol]) / self.entry_prices[symbol]
                if current_pos < 0:
                    tighten_pnl = -tighten_pnl
                _apt = RSI_EXIT_PROFIT_THRESHOLD * max(0.7, min(1.4, vol_ratio ** 0.5))
                if tighten_pnl > _apt:
                    _pb = min(RSI_EXIT_PROFIT_TIGHTEN * (1.0 + 0.50 * min(1.0, max(0.0, (0.70 - vol_ratio) / 0.15))), (tighten_pnl - _apt) * RSI_EXIT_PROFIT_SCALE / max(0.6, min(1.8, vol_ratio)))
