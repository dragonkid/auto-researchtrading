    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural simplification: removed _chop_amp own-side amplification
                # AND _opp_atten trend-aligned opp-bias attenuation. Both were ad-hoc
                # cross-timescale modulations layered on top of bilateral additive
                # voter_bias. Base bilateral fusion (-0.20 own + 0.20 opp) is the
                # primary mechanism; the modulations were calibrated to recover marginal
                # regime gains but add LOC and correlated-noise channels (both depend
                # on |ret_long| or pos-aligned ret_long). Code-structure removal:
                # plain bilateral fusion with constant weights.
