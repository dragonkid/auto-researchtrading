...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Multi-point trend_avg: 3-bar averages at both endpoints reduce zero-crossing noise
            _trend_recent = np.mean(closes[-3:])
            _trend_past = np.mean(closes[-MED2_WINDOW - 1: -MED2_WINDOW + 2])
            _trend_med = (_trend_recent - _trend_past) / _trend_past
            trend_avg = (TREND_GATE_MED_WEIGHT_SIDEWAYS - (TREND_GATE_MED_WEIGHT_SIDEWAYS - TREND_GATE_MED_WEIGHT_BASE) * cooldown_trend_strength) * _trend_med + ((1.0 - TREND_GATE_MED_WEIGHT_SIDEWAYS) + (TREND_GATE_MED_WEIGHT_SIDEWAYS - TREND_GATE_MED_WEIGHT_BASE) * cooldown_trend_strength) * ret_long
