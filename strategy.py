...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Noise-immune strength: use linreg slope magnitude (16-bar multi-point) instead of ret_short
            # Each perturbed bar contributes 1/16th to slope vs 1/1 for ret_short single-point
            _linreg_strength = min(2.0, (abs(_lr.slope) / 0.0004) ** 0.85)
            strength_scale = max(0.6 + (STRENGTH_FLOOR_SIDEWAYS - 0.6) * (1.0 - min(abs(ret_long) / STRENGTH_FLOOR_DECAY, 1.0)), _linreg_strength)
