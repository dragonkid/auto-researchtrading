        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if target != 0:
                    ...
                    if self.peak_pnl[symbol] > PEAK_PROFIT_MIN_BASE * max(0.6, min(2.0, vol_ratio ** 0.5)) and self.peak_pnl[symbol] - _spp > self.peak_pnl[symbol] * PEAK_PROFIT_GIVEBACK:
                        target = 0.0
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self.entry_vol_ratio, self.smoothed_pos_pnl):
                        _d.pop(symbol, None)
                    self.exit_bar[symbol] = self.bar_count
                elif current_pos == 0 or (target > 0 and current_pos < 0) or (target < 0 and current_pos > 0):
                    self.entry_prices[symbol], self.peak_pnl[symbol], self.entry_bar[symbol] = mid, 0.0, self.bar_count
                    self.entry_vol_ratio[symbol], self.smoothed_pos_pnl[symbol] = vol_ratio, 0.0
