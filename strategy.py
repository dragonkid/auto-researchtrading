    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Flip mechanism (votes + trend_avg sign WITH deadzone, vol-scaled, confidence-sized)
                # Deadzone: block flips when trend_avg is near zero (noise-sensitive sign)
                _flip_trend_clear = abs(trend_avg) > TREND_GATE_DEADZONE
                if not in_cooldown and _flip_trend_clear and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and trend_avg > 0)):
