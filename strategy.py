...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            lr_entry_gate = rsi_trend_str <= 0.5
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (lr_entry_gate or _lr.slope > 0) and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and (lr_entry_gate or _lr.slope < 0) and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
