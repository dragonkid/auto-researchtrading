...
class Strategy:
    def __init__(self):
        ...
        self.bar_count = 0
        self.smoothed_trend = {}
        self.last_bull_votes, self.last_bear_votes = {}, {}
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _last_bull, _last_bear = self.last_bull_votes.get(symbol, 0), self.last_bear_votes.get(symbol, 0)
            self.last_bull_votes[symbol], self.last_bear_votes[symbol] = bull_votes, bear_votes
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and _last_bull >= MIN_VOTES and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and _last_bear >= MIN_VOTES and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
