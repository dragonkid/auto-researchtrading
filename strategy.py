...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # RSI computed on HL2 for noise reduction: HL2 receives ~50% perturbation vs close
            # RSI is a voter (not exit), so slight lag from HL2 smoothing is acceptable
            _rsi_hl2 = (bd.history["high"].values[-(int(round(6 + 2 * rsi_trend_str)) + 2):] + bd.history["low"].values[-(int(round(6 + 2 * rsi_trend_str)) + 2):]) / 2.0
            _rd = np.diff(_rsi_hl2)
