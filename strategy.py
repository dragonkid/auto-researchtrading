...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _rl_norm = min(abs(ret_long) / STRENGTH_FLOOR_DECAY, 1.0)
            strength_scale = STRENGTH_FLOOR_SIDEWAYS - (STRENGTH_FLOOR_SIDEWAYS - 0.6) * _rl_norm + 1.4 * _rl_norm
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                _apt = RSI_EXIT_PROFIT_THRESHOLD * max(0.7, min(1.4, _evr ** 0.5))
                if pos_pnl > _apt:
