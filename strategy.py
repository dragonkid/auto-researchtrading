    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Flip mechanism (votes + trend_avg sign, fixed 75% fraction for noise stability)
                # Fixed at 0.75 removes vol_ratio->frac noise channel; 75% balances crash protection vs stability
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and trend_avg > 0)):
                    target = (-_conf_size if current_pos > 0 else _conf_size) * 0.75
