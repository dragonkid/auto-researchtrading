    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (bull_votes - bear_votes) >= 2 and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size
                elif bear_votes >= MIN_VOTES and (bear_votes - bull_votes) >= 2 and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size
                ...
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and (bear_votes - bull_votes) >= 3 and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and (bull_votes - bear_votes) >= 3 and trend_avg > 0)):
