...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Architectural simplification: removed Kaufman efficiency ratio (_er) gate
            # on _entry_frac_dyn. The deep-chop suppression overlaps with: vol_ratio
            # (already shrinks _entry_frac_dyn via tanh), _confluence_adj (gates on slope
            # agreement, which is itself an efficiency proxy), and the strong-sum gate
            # (filters chop noise via voter consensus). Removing one redundant gate.
