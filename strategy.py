...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # In calm (sideways): use raw closes for fast response (linreg's multi-point fit is already noise-robust)
            # In volatile: use smoothed_closes for extra filtering
            _lr_short_input = closes[-_lr_short_window:] if vol_ratio < 0.8 else smoothed_closes[-_lr_short_window:]
            _lr_short = linregress(np.arange(_lr_short_window), np.log(_lr_short_input))
            ...
            # 6 voters: EMA cross replaced with short linreg slope (multi-point fit, noise-immune)
            bull_votes = sum([ret_short > dyn_threshold, _lr_short.slope > 0.0001, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, _lr_short.slope < -0.0001, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
