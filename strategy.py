...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _vc_trim_recent = (np.sum(_vol_recent) - np.max(_vol_recent)) / (len(_vol_recent) - 1)
            _vc_trim_base = (np.sum(_vol_base) - np.max(_vol_base)) / (len(_vol_base) - 1)
            vol_confirm_mult = max(VOL_CONFIRM_FLOOR, min(VOL_CONFIRM_CAP, _vc_trim_recent / _vc_trim_base))
