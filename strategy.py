    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # attenuator on first-bar SIZE. The 3-window (8/16/32) slope alignment
                # computed via tanh-average mapped to [0.40, 1.00] — up to 60% size
                # attenuation on slope-disagreeing entries. Redundant with: (a) 2-bar
                # persistence gate (filters single-bar noise entries at admission level),
                # (b) quality attenuator (bilateral voter split on same entry signal),
                # (c) trend gate DEADZONE (trend_avg against position direction already
                # filters counter-slope entries). Three slope computations + 3 cross-bar
                # HL2 reads removed. Code-structure removal: -17 lines + -3 slope OLS.
