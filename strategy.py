...
MIN_VOTES = 4
FLIP_MIN_VOTES = 4
VOTE_NET_MARGIN = 3  # minimum bull_votes - bear_votes gap for entry (noise margin on aggregate decision)
COOLDOWN_BARS = 1
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (bull_votes - bear_votes) >= VOTE_NET_MARGIN and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    target = size * ENTRY_INITIAL_FRAC
                elif bear_votes >= MIN_VOTES and (bear_votes - bull_votes) >= VOTE_NET_MARGIN and (self.smoothed_trend[symbol] < 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bear_votes > bull_votes)):
                    target = -size * ENTRY_INITIAL_FRAC
                ...
                # Flip mechanism (4 votes + trend_avg sign + net margin, vol-scaled accumulation)
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and (bear_votes - bull_votes) >= VOTE_NET_MARGIN and trend_avg < 0) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and (bull_votes - bear_votes) >= VOTE_NET_MARGIN and trend_avg > 0)):
