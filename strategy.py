    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # size attenuator). Used current bar volume vs 24-bar avg to cut first-bar
                # size up to 30% on thin-volume entries. Redundant with the now-removed
                # _vol_amp aggregation amplifier — both used the same volume-vs-24bar-avg
                # signal at different stages. After removing _vol_amp, the entry path retains
                # multiple stacked attenuators (_ct_atten, _concurrent_atten, _consensus_atten,
                # _quality_atten) that already filter noise-vulnerable entries; the volume
                # condition was orthogonal in principle but in practice low-volume bars
                # tend to also be deep-chop bars that _consensus_atten/_quality_atten already
                # attenuate. Removing one orthogonal-on-paper but practically-redundant
                # attenuator. Code-structure removal: 17 lines + 1 cross-bar volume read.
