...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _rd_exit_prev = np.diff(closes[-8:-1])
            _rsi_exit_prev = 100 - 100 / (1 + np.mean(np.maximum(_rd_exit_prev, 0)) / max(np.mean(np.maximum(-_rd_exit_prev, 0)), 1e-10))
            rsi_exit = 0.7 * _rsi_exit_raw + 0.3 * _rsi_exit_prev
