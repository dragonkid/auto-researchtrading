...
# Sizing multipliers
BASE_POSITION_SIZE = 0.072  # increased to compensate for fewer entries (meanrev removal)
CALM_BOOST_MAX = 0.8
...
ENTRY_INITIAL_FRAC = 0.40  # reduced from 0.43: less first-bar noise exposure
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    ...
                    target = -size * ENTRY_INITIAL_FRAC
                # Mean-reversion path removed: RSI 49/51 near neutral is noise-sensitive
            elif current_pos != 0:
