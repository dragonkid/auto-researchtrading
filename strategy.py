...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Asymmetric EMA: bear side uses EMA_FAST=4 (smoother, less noise-sensitive cross)
            # Bull side keeps EMA_FAST=3 (faster entry detection)
            _ef_bear = ema(closes[-(EMA_SLOW+10):], 4)[-1]  # bear: span=4 for smoother cross

            # 6 voters (bear EMA cross uses slightly smoother fast EMA)
            ...
            bear_votes = sum([ret_short < -dyn_threshold, _ef_bear < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
