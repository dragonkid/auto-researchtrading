...
class Strategy:
    def __init__(self):
        ...
        # Exp4 (architectural iteration on Exp3 keep): sustain the volume-DECLINE entry
        # shrink through scale-in (cached at entry, deterministic). Mirrors the proven
        # Exp9 mechanism (vol-spike sustain KEEP). Keeps a low-participation entry smaller
        # for the whole hold instead of ramping back to un-shrunk `size` after bar 1 ->
        # smaller losses on low-participation trades -> higher Sharpe. A SHRINK sustained
        # (safe family). Default 1.0 (no effect) if uncached.
        self._vol_decline_held = {}
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if _bull_ready and _bull_admit:
                    ...
                    self._vol_decline_held[symbol] = _vol_decline_shrink  # Exp4: cache for scale-in sustain
                ...
                if bars_held <= _entry_full_bars_dyn:
                    ...
                    # Exp4: sustain the Exp3 volume-DECLINE entry shrink through scale-in.
                    _vol_decl_held = self._vol_decline_held.get(symbol, 1.0)
                    full_target = (size if current_pos > 0 else -size) * _conc_held * _vol_held * _vol_decl_held
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    ...
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self._smoothed_pnl, self._mae, self._exit_press_ema, self._voter_bias_ema, self._target_ema, self._conc_shrink_held, self._vol_shrink_held, self._vol_decline_held):
                        _d.pop(symbol, None)
