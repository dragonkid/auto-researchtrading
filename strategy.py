...
MIN_VOTES = 3
FLIP_MIN_VOTES = 4
FLIP_LINREG_BASE = 0.0004
FLIP_LINREG_TREND_SCALE = 0.002
FLIP_LINREG_TREND_SATURATE = 0.10
COOLDOWN_BARS = 3
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                _flip_thresh = FLIP_LINREG_BASE + FLIP_LINREG_TREND_SCALE * min(abs(ret_long) / FLIP_LINREG_TREND_SATURATE, 1.0)
                if not in_cooldown and ((current_pos > 0 and _lr.slope < -_flip_thresh) or (current_pos < 0 and _lr.slope > _flip_thresh)):
