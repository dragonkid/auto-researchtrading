    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                _flip_th = 0.005 + 0.007 * min(1.0, abs(ret_long) / 0.10)
                if not in_cooldown and ((current_pos > 0 and bear_votes >= FLIP_MIN_VOTES and (self.smoothed_trend[symbol] < 0 or trend_avg < -_flip_th)) or (current_pos < 0 and bull_votes >= FLIP_MIN_VOTES and (self.smoothed_trend[symbol] > 0 or trend_avg > _flip_th))):
