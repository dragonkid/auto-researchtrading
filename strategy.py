...
# Per-voter abstain zones (fraction of threshold where voter abstains)
ABSTAIN_FRAC = 0.15  # signal within threshold*(1-frac) to threshold*(1+frac) = abstain
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Per-voter abstain zones: signal must exceed threshold*(1+ABSTAIN_FRAC) to vote
            # Signals within threshold*(1-ABSTAIN_FRAC) to threshold*(1+ABSTAIN_FRAC) abstain
            _af = ABSTAIN_FRAC
            _ret_s_thr = dyn_threshold
            _ret_vs_thr = dyn_threshold * 0.70
            ...
            bull_votes = sum([
                ret_short > _ret_s_thr * (1 + _af),
                ret_vshort > _ret_vs_thr * (1 + _af),
                (_ef - _es) / max(abs(_es), 1e-10) > 0.0002 * (1 + _af),
                rsi > _rsi_thr + RSI_TREND_BIAS * _af,
                _macd_val > 0.0003 * (1 + _af),
                _lr.slope > 0.00015 * (1 + _af),
                _ema_slope_val > 0.0005 * (1 + _af),
            ])
            bear_votes = sum([
                ret_short < -_ret_s_thr * (1 + _af),
                ret_vshort < -_ret_vs_thr * (1 + _af),
