    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural simplification: removed _chop_amp (chop-amplified own-side
                # subtraction). Prior session hint: joint removal of _chop_amp+_opp_atten
                # gave sideways +0.019. Joint removal collapsed crash/rally because _opp_atten
                # is load-bearing. Removing _chop_amp ALONE while keeping _opp_atten tests
                # whether _chop_amp was a slight DRAG on sideways (over-amplifying own-side
                # subtraction in chop where mean-reverting voters legitimately validate hold).
