...
MEANREV_RSI_DEADZONE = 0.5
MEANREV_RSI_FULL_RANGE = 5.0
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            funding_rate = bd.history["funding_rate"].values[-1]
            bull_votes = sum([ret_short > dyn_threshold, ret_vshort > dyn_threshold * 0.70, _ef > _es, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, mid > np.max(closes[-(DONCHIAN_PERIOD+1):-1]) * 1.004, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005, funding_rate > FUNDING_VOTE_THRESHOLD])
            bear_votes = sum([ret_short < -dyn_threshold, ret_vshort < -dyn_threshold * 0.70, _ef < _es, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if ret_long > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, mid < np.min(closes[-(DONCHIAN_PERIOD+1):-1]) * 0.9975, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005, funding_rate < -FUNDING_VOTE_THRESHOLD])
            ...
            if current_pos == 0 and not in_cooldown:
                if bull_votes >= MIN_VOTES and (self.smoothed_trend[symbol] > 0 or (abs(self.smoothed_trend[symbol]) < TREND_GATE_DEADZONE and bull_votes > bear_votes)):
                    ...
                    target = -size
                elif abs(ret_long) < MEANREV_TREND_THRESHOLD and abs(rsi - 50.0) > MEANREV_RSI_DEADZONE:
                    mr_strength = max(0.0, min(1.0, (abs(rsi - 50.0) - MEANREV_RSI_DEADZONE) / MEANREV_RSI_FULL_RANGE))
                    target = (size * mr_strength) * (-1.0 if rsi > 50.0 else 1.0)
            elif current_pos != 0:
