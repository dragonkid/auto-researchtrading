...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _ema_f = ema(closes[-(EMA_SLOW+10):], EMA_FAST)
            _ema_s = ema(closes[-(EMA_SLOW+10):], EMA_SLOW)
            _ef, _es = _ema_f[-1], _ema_s[-1]
            _ef_prev, _es_prev = _ema_f[-2], _ema_s[-2]
            ...
            bull_votes = sum([ret_short > dyn_threshold, ret_vshort > dyn_threshold * 0.70, _ef_prev > _es_prev, rsi > 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if _ret_long_lagged > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid > 0.0003, _lr.slope > 0.00015, mid > np.max(closes[-(DONCHIAN_PERIOD+1):-1]) * 1.004, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] > 0.0005])
            bear_votes = sum([ret_short < -dyn_threshold, ret_vshort < -dyn_threshold * 0.70, _ef_prev < _es_prev, rsi < 50 + RSI_TREND_BIAS * rsi_trend_str * (-1.0 if _ret_long_lagged > 0 else 1.0), (_ml[-1] - ema(_ml, MACD_SIGNAL)[-1]) / mid < -0.0003, _lr.slope < -0.00015, mid < np.min(closes[-(DONCHIAN_PERIOD+1):-1]) * 0.9975, (_ea[-1] - _ea[-EMA_SLOPE_LOOKBACK]) / _ea[-EMA_SLOPE_LOOKBACK] < -0.0005])
