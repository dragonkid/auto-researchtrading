...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            _bull_strong_min += 0.20 * _strong_min * max(0.0, min(1.0, (_bear_strong - 0.5 * _strong_min) / max(0.5 * _strong_min, 1e-6)))
            _bear_strong_min += 0.20 * _strong_min * max(0.0, min(1.0, (_bull_strong - 0.5 * _strong_min) / max(0.5 * _strong_min, 1e-6)))
