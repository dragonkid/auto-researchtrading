    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural simplification: removed _tod_atten (6-frequency
                # timestamp cyclic product on first-bar entry size).
                # Old: TOD(24h) × DOW(7d) × MOM(30d) × QUARTER(91d) × SEMI(180d) × YEAR(365d)
                # compounded into [0.85, 1.15] first-bar size multiplier. Six overlapping
                # cyclic harmonics fit calendar noise patterns specific to the 4-regime
                # backtest period (selection bias on cycle-phase alignment). Each harmonic's
                # contribution is structurally smaller than the prior, with the year-cycle
                # adding only ~1pct amplitude — yet they all stack multiplicatively.
                # The ±15pct range is small but the harmonic stack is overfit-prone:
                # the more cycles, the more knobs to fit calendar artifacts. Removing
                # eliminates the timestamp data source from entry size, leaving size
                # determined by conviction/quality factors only.
                # Code-structure removal: -12 LOC + timestamp data dependency removed.
                _tod_atten = 1.0
