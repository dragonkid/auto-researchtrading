...
class Strategy:
    def __init__(self):
        ...
        self.bar_count = 0
        self.smoothed_trend = {}
        self.exit_slope_confirm = {}  # tracks consecutive bars with adverse slope
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Vol-adaptive linreg exit with 2-bar confirmation (hysteresis):
                # Require adverse slope for 2 consecutive bars to filter single-bar noise
                ...
                if _slope_adverse:
                    self.exit_slope_confirm[symbol] = self.exit_slope_confirm.get(symbol, 0) + 1
                else:
                    self.exit_slope_confirm[symbol] = 0
                if target != 0 and self.exit_slope_confirm.get(symbol, 0) >= 2:
                    target = 0.0
            ...
            if abs(target - current_pos) > 1.0:
                ...
                if target == 0:
                    for _d in (self.entry_prices, self.peak_pnl, self.entry_bar, self.exit_slope_confirm):
                        _d.pop(symbol, None)
