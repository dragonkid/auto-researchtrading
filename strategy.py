...
VOTE_SIZE_BASE = 4          # votes at which multiplier = 1.0
VOTE_SIZE_SCALE = 0.15      # mult change per vote above/below base
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            active_votes = max(bull_votes, bear_votes)
            vote_size_mult = 1.0 + VOTE_SIZE_SCALE * (active_votes - VOTE_SIZE_BASE)
            combined_mult = max(0.3, min(2.5, (TARGET_VOL / realized_vol) ** 0.85)) * strength_scale * calm_boost * sideways_boost * (1.0 + CROSS_ASSET_FIXED_BOOST * (1.0 - cooldown_trend_strength)) * vote_size_mult * vol_confirm_mult
