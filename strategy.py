...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # 3-bar mean of recent smoothed closes (reduces single-bar noise at comparison point)
            _recent_avg = np.mean(smoothed_closes[-3:])
            ret_vshort = (_recent_avg - _med_ref_short) / _med_ref_short
            ret_short = (_recent_avg - _med_ref_med) / _med_ref_med
