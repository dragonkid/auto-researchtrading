        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                if target != 0 and self.peak_pnl[symbol] > 1.6 * _pp_min and _sl_pressure < 0.5:
                    ...
                    _mae_clean = 1.0 - max(0.0, min(1.0, np.tanh(_mae_depth / 0.3)))
                    _pos_dir_tp = 1.0 if current_pos > 0 else -1.0
                    _trend_align_tp = max(0.0, np.tanh(ret_long * _pos_dir_tp / 0.04))
                    _tp_scale = 0.30 * max(0.0, min(1.0, np.tanh((_tp_ratio - 1.6) / 0.6))) * _tp_trend_gate * (1.0 - 0.5 * _mae_clean * _trend_align_tp)
