    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: volume-conviction flip gate. Flip win rate is 6-14% across
                # regimes (flip_pnl deeply negative in rally -829). Hypothesis: low-volume
                # flips are noise-driven whipsaws; require recent volume to confirm a real
                # reversal before tightening the strong-sum requirement. New data dependency
                # in flip path: volume primitive used to scale flip strong-min (was unused
                # in flip subsystem prior). Continuous tanh — when recent volume is below
                # baseline, flip threshold rises proportionally; above baseline, no penalty.
                _flip_vol_short = np.mean(_vols[-4:])
                _flip_vol_base = np.mean(_vols[-20:])
                _vol_weakness = max(0.0, 1.0 - _flip_vol_short / max(_flip_vol_base, 1e-6))
                _vol_flip_factor = 0.18 * np.tanh(_vol_weakness / 0.20)  # 0..0.18
                _bull_flip_min = _bull_strong_min * (1.0 + 0.20 * _flip_recency_factor + _vol_flip_factor)
                _bear_flip_min = _bear_strong_min * (1.0 + 0.20 * _flip_recency_factor + _vol_flip_factor)
