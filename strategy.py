    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # EMA-strong-sum confluence on flip path: when new-side EMA is well below
                # _strong_min, recent buildup is absent — require higher vote count smoothly.
                # Continuous: penalty up to +0.20 when new-side EMA ratio < 0.5*_strong_min.
                _flip_new_ema = (_bull_ema if current_pos < 0 else _bear_ema)
                _flip_ema_ratio = _flip_new_ema / max(_strong_min, 1e-6)
                _flip_ema_penalty = 0.20 * max(0.0, min(1.0, (0.5 - _flip_ema_ratio) / 0.4))
                _flip_min_eff = FLIP_MIN_VOTES + _flip_ema_penalty
