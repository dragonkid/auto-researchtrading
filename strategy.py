...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Exp8 (architectural, indep): 9th voter -- 48-BAR range/close efficiency
            # (longer-timescale version of the 8th voter). The 12-bar 8th voter helped
            # rally/bull (low-vol trends) but was byte-identical on crash (high-vol: the
            # 12-bar intrabar ranges are large in crash, diluting the ratio). A 48-bar
            # window averages more of the sustained trend -> the multi-day trend move
            # dominates the intrabar noise -> efficiency > 1 in sustained trends at EITHER
            # vol level. Captures multi-day trend QUALITY (orthogonal timescale to the
            # 12-bar 8th voter; both are efficiency-QUALITY not direction, respecting the
            # Exp6 lesson that directional voters over-amplify). Signed by 48-bar close
            # direction. Small fixed weight 0.45 (below the 8th's 0.55).
            _rc_n2 = 48
            _rc_high2 = bd.history["high"].values[-_rc_n2:]
            _rc_low2 = bd.history["low"].values[-_rc_n2:]
            _rc_intrabar2 = float(np.mean(_rc_high2 - _rc_low2))
            _rc_interbar2 = float(np.mean(np.abs(np.diff(closes[-_rc_n2 - 1:]))))
            _rc_eff2 = _rc_interbar2 / max(_rc_intrabar2, 1e-10)
            _rc_dir2 = 1.0 if closes[-1] >= closes[-_rc_n2] else -1.0
            _rc_signal2 = (_rc_eff2 - 1.0) / 0.5 * _rc_dir2
            _voter_signals_bull = [
                ...
                _rc_signal2 / 1.0,  # Exp8: 9th voter 48-bar range/close efficiency (longer timescale)
            ]
            ...
            _base_weights = (0.7, 1.25 + _wt_shift, 1.10 - _wt_shift, 1.00 - _wt_shift, 0.85, 1.10 + _wt_shift, _vwap_wt, 0.55, 0.45)  # 8th range/close 12-bar, 9th range/close 48-bar (Exp8)
