...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            bypass_level_ok_bull = not trend_gate_bypassed or mid <= ema_slow_arr[-1]
            bypass_level_ok_bear = not trend_gate_bypassed or mid >= ema_slow_arr[-1]
            bullish = bull_votes >= MIN_VOTES and (trend_bull or (trend_gate_bypassed and bypass_level_ok_bull))
            bearish = bear_votes >= MIN_VOTES and (trend_bear or (trend_gate_bypassed and bypass_level_ok_bear))
