...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Removed strength_scale: noise channel from ret_short was inflating size during noisy
            # short-term moves. Architectural simplification — sizing now driven only by vol/calm/sideways
            # gates which are smoother and less noise-sensitive than ret_short magnitude.
            combined_mult = max(0.3, min(2.5, (TARGET_VOL / realized_vol) ** 0.85)) * calm_boost * sideways_boost * (1.0 + CROSS_ASSET_FIXED_BOOST * (1.0 - cooldown_trend_strength)) * HIGH_VOTE_BOOST_MULT * vol_confirm_mult
