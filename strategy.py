...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            vol_above_avg = np.mean(bd.history["volume"].values[-3:]) > np.mean(bd.history["volume"].values[-VOL_CONFIRM_BASE:])
            vol_breakout_bull = vb_short > realized_vol and ret_vshort > 0 and vol_above_avg
            vol_breakout_bear = vb_short > realized_vol and ret_vshort < 0 and vol_above_avg
