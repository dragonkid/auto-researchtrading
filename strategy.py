...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Simplified sizing: remove noise-sensitive components (strength_scale, vol_confirm_mult)
            # Keep only: vol-targeting + calm_boost + cross-asset + trend cap
            # Rationale: strength_scale uses ret_short/dyn_threshold (directly noise-exposed),
            # vol_confirm uses volume ratio (less noise-sensitive but adds complexity)
            ...
            # Fixed strength scale using ret_long (20-bar, noise-diluted) instead of ret_short
            strength_scale_stable = max(1.0, min(2.0, (abs(ret_long) / 0.05) ** 0.5))
            combined_mult = max(0.3, min(2.5, (TARGET_VOL / realized_vol) ** 0.85)) * strength_scale_stable * calm_boost * sideways_boost * (1.0 + CROSS_ASSET_FIXED_BOOST * (1.0 - cooldown_trend_strength)) * HIGH_VOTE_BOOST_MULT
