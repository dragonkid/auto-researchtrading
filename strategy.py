    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            if current_pos == 0 and not in_cooldown:
                ...
                # Architectural: trend-continuation override on stale-position exit signals.
                # When slope agrees with position direction AND slope strength is high,
                # reduce the weight of peak-profit AND time pressure (the two stale-position
                # exit channels). Slope-against weight is NOT reduced (loss-cutting preserved).
                # Stop-loss is NOT reduced (protective gate preserved).
                # New cross-coupling primitive: signal-A (slope-with strength) modulates
                # the WEIGHTS of signals B/C in exit fusion. Continuous tanh on slope
                # strength bounded [0, 0.20] reduction. Different from prior pnl-asymmetric
                # weight experiments which used pnl as the modulator — here the modulator
                # is the slope's directional strength, an external signal.
                _slope_with_strength = _slope_strength if _slope_agrees else 0.0
                _trend_continue = 0.20 * np.tanh(_slope_with_strength / 0.5)
                _w_pp_eff = _w_pp * (1.0 - _trend_continue)
                _w_time_eff = _w_time * (1.0 - _trend_continue)
                _exit_pressure = _sl_pressure + _w_slope * _sl_slope_pressure + _w_pp_eff * _pp_pressure + _w_time_eff * _time_pressure
