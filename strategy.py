...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Simplified sizing: vol-targeting only (architectural: remove all noise-carrying sub-multipliers)
            # calm_boost, sideways_boost, strength_scale, vol_confirm, cross_asset all inject noise
            # via their dependency on close-bar-derived values (vol ratios, ret_short, ret_long)
            # Pure vol-targeting: size inversely proportional to realized vol, capped
            _vol_mult = max(0.4, min(3.0, (TARGET_VOL / realized_vol) ** 0.85))
            size = equity * 0.12 * _vol_mult
