...
ENTRY_GATE_CENTER_BASE = 2.1  # sigmoid center (slightly above baseline 2.0)
ENTRY_GATE_CENTER_VOL_BOOST = 0.1  # extra center in calm for noise protection
...
ENTRY_INITIAL_FRAC = 0.55  # first bar: 55% of target (baseline)
...
class Strategy:
    ...
    def on_bar(self, bar_data, portfolio):
        ...
        for symbol in ACTIVE_SYMBOLS:
            ...
            # Vol-adaptive center: higher in calm (more conservative sizing) for noise protection
            _vol_center_factor = max(0.0, min(1.0, (1.0 - vol_ratio) / 0.5))  # 1.0 when vol_ratio<=0.5, 0.0 when vol>=1.0
