...
6. **Record** (mandatory — do NOT skip): Append one row to `results.tsv` for EVERY experiment. This is not optional.
   ...
   **Keep/discard rules:**
   ...
   - `raw_composite` ≥ **10.10** (must not degrade below current baseline floor). Calibrated to current best strategy 62e6952 (raw 10.78).
...
- The ONLY hard constraints are: DD caps (bull ≤7.8%, crash ≤6.9%, sideways ≤5.6%, rally ≤6.0%), raw_composite ≥ 10.10, and min_stab ≥ baseline - 0.002
