...
6. **Record** (mandatory — do NOT skip): Append one row to `results.tsv` for EVERY experiment. This is not optional.
   ...
   **Keep/discard rules:**
   ...
   - `raw_composite` ≥ **9.40** (must not degrade below current baseline floor). Calibrated to current best strategy 23e541c (raw 9.97).
...
- The ONLY hard constraints are: DD caps (bull ≤7.8%, crash ≤6.9%, sideways ≤5.6%, rally ≤6.0%), raw_composite ≥ 9.40, and min_stab ≥ baseline - 0.002
