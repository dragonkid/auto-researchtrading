...
- **Failure (revert the branch)**: if the branch terminates without meeting keep criteria (either via 7 consecutive no-progress steps or because you decided to stop), revert ALL branch commits back to the original baseline: `git revert --no-edit HEAD~N..HEAD` (where N = number of branch commits). **CRITICAL: before reverting, check `git log --oneline HEAD~N..HEAD` and ensure you are ONLY reverting your own experiment commits (messages starting with "exp:" or "branch step"). If any commits in that range have different prefixes (e.g., "feat:", "fix:", "doc:"), they are infrastructure changes made by the project owner — do NOT revert them. Reduce N to skip those commits, or revert individually.** Record ONE summary `discard` line explaining the branch attempt and why it failed.
