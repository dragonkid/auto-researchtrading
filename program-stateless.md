...
Each regime is scored individually via `compute_score()` in `prepare.py`, then combined:

```
Per-regime score = w_sharpe * log(1+sharpe) * sqrt(trade_count_factor)
                 + w_return * log(1+return%)
                 - w_dd * DD²
...
High mean = good overall. Low std = stable across bull/bear/sideways/crash.
...
A strategy that scores great in one regime but poorly in others will have HIGH std, dragging down the composite.
