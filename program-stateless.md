...
8. **Record**: If `composite_score` improved vs the best in `results.tsv`, append a `keep` line. If worse or equal, run `git checkout HEAD~1 -- strategy.py && git commit -am "revert: <description>"` to undo only strategy.py (preserves prepare.py and other harness files), then append a `discard` line. NEVER use `git reset --hard` — it destroys non-strategy changes.
